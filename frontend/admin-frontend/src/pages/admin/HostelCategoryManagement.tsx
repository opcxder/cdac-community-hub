...
import type { HostelCategory } from '@/types';
import { useToast } from '@/hooks/use-toast';

/**
 * Hostel Category Management Page
 * Admin page for approving/rejecting hostel category suggestions
 */
export function HostelCategoryManagement() {
    const [categories, setCategories] = useState<HostelCategory[]>([]);
    const [loading, setLoading] = useState(true);
    const [selectedCategory, setSelectedCategory] = useState<HostelCategory | null>(null);
    ...
    const fetchCategories = async () => {
        try {
            setLoading(true);
            const data = await adminService.getPendingHostelCategories();
            setCategories(data);
        } catch (error) {
            toast({
                title: 'Error',
    ...
    const handleApprove = async (categoryId: number) => {
        try {
            ...
            await adminService.approveHostelCategory(categoryId);
            toast({
                title: 'Success',
    ...
    const handleRejectClick = (category: HostelCategory) => {
    ...
    const handleRejectConfirm = async () => {
        ...
        try {
            setActionLoading(true);
            await adminService.rejectHostelCategory(selectedCategory.categoryId, rejectionReason);
            toast({
                title: 'Success',
    ...
    return (
        <div className="container mx-auto py-6">
            <div className="mb-6">
                <h1 className="text-3xl font-bold">Hostel Category Management</h1>
                <p className="text-muted-foreground">
                    Review and approve/reject hostel category suggestions from users
                </p>
            </div>
            ...
            {categories.length === 0 ? (
                <EmptyState
                    ...
                    description="All hostel category suggestions have been reviewed"
                />
            ) : (
